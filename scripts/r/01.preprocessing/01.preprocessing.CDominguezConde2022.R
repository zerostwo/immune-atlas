...
outdir <- sn_set_path("data/processed/CDominguezConde2022/corrected_soupx")

dirs <- sn_list_10x_paths("/home/sduan/data/CDominguezConde2022", path_type = "filtered_h5")
samples <- names(dirs)
raw <- sn_list_10x_paths("/home/sduan/data/CDominguezConde2022", path_type = "raw_h5")
# samples <- list.files(file.path(outdir, "seurat_object"), pattern = "qs") %>% str_remove_all(".qs")

samples <- samples[!(
  samples %in% c(
    "A52-TLN-0-SC-1", # Extremely high contamination estimated (0.67).
    "A29-SKM-0-SC-1",
    "A37-SKM-0-SC-1",
    "A36-SKM-0-SC-1",
    "A37-DUO-0-SC-1"
...
for (sample in samples) {
  ...
  if (!file.exists(so_path)) {
    ...
    seurat_obj <- sn_remove_ambient_contamination(seurat_obj, method = "soupx", raw = raw[sample])
