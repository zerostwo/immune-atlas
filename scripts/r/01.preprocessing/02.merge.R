...
MarylineFalquet2023 <- merge(x = MarylineFalquet2023[[1]],
...
AndrewDHildreth2021 <- merge(x = AndrewDHildreth2021[[1]],
...
CDominguezConde2022 <- merge(x = CDominguezConde2022[[1]],
...
NataliaJaeger2024 <- merge(x = NataliaJaeger2024[[1]],
...
merged <- JoinLayers(merged, layers = "counts", assay = "ADT")
merged@assays$HTO <- NULL
merged
table(merged$study)
sum(rowSums(LayerData(CDominguezConde2022, "counts")))
sum(rowSums(LayerData(CDominguezConde2022, "decontaminated_counts")))
...
LayerData(merged, "data.17.3") <- NULL
LayerData(merged, "scale.data.17.3") <- NULL
merged
...
# Merge -------------------------------------------------------------------
merged1 <- sn_read(
  "data/processed/AndrewDHildreth2021_CDominguezConde2022_NataliaJaeger2024_MarylineFalquet2023_ShuaiHe2020_DanielPCaron2025_EMadissoon2020/merged.qs"
)
merged2 <- sn_read("data/processed/ZhenlongLi2024_StevenBWells2025_TabulaSapiens2022/merged.qs")
...
counts <- sn_read(
  "data/processed/ZhenlongLi2024_StevenBWells2025_TabulaSapiens2022/bpcells/decontaminated_counts/"
)
...
sn_write(merged, "data/processed/20260410.reference.raw.qs")

table(merged$scDblFinder.class_corrected_soupx, merged$study) |>
  as.data.frame() |>
...
table(merged$study)
table(merged$scDblFinder.class_corrected_soupx)
